...
# Préparer une colonne pour les associations préliminaires
Commu$D_large$assoc.prelim <- NA
```
...
# Sh-Gii/Pj : Communautés cryoturbées à Salix herbacea - Gymnocolea inflata subsp. inflata/Polytrichum juniperinum
...
## Attribuer le code du groupe
Commu$D_large$assoc.prelim[groupes == 1] <- "Sh-Gii/Pj"
```

```{r}
table(Commu$D_large$assoc.prelim)
```
...
p_NMDS_12 <- Commu$D_large |> 
  filter(complete.cases(assoc.prelim)) |> 
  group_by(assoc.prelim) |> 
  ...
  geom_point(data = Commu$D_large,
             aes(color = drain.class.1), alpha = 0.5, size = 2) + 
...
p_NMDS_34 <- Commu$D_large |> 
  ...
  geom_point(data = Commu$D_large,
             aes(color = drain.class.1), alpha = 0.5, size = 2) + 
  geom_label(aes(x = med.NMDS3.pam, y = med.NMDS4.pam, label = assoc.prelim),
             size = 3) + 
  theme_minimal()
...
```{r}
Table_forme_croissance <- table_formes_croissance(Commu, "assoc.prelim")

Table_forme_croissance |> 
  filter(assoc.prelim == "Sh-Gii/Pj") |> 
...
```{r}
Table_sp <- table_sp(Commu, "assoc.prelim")

Table_sp |> 
  filter(assoc.prelim == "Sh-Gii/Pj", occ.class != "I") |> 
...
# Vu/Enh-Aj : Communautés cryoturbées à Vaccinium uliginosum/Empetrum nigrum subsp. hermaphroditum - Anthelia juratzkana
...
## Attribuer le code du groupe
Commu$D_large$assoc.prelim[groupes == 15] <- "Vu/Enh-Aj"
```
...
p_freq <- Table_forme_croissance |> 
  filter(assoc.prelim %in% c("Sh-Gii/Pj", "Vu/Enh-Aj")) |> 
...
Table_sp |> 
  filter(assoc.prelim == "Vu/Enh-Aj", occ.class != "I") |> 
  arrange(sous.regne, desc(occ.pourc)) |> 
  select(sp.nom, occ.class, couvert) |> 
  tt() |> 
  setNames(c("Espèce", "Occurence", "Couvert")) |> 
...
```{r}
Table_enviro <- table_enviro(Commu, "assoc.prelim")
...
```{r}
Commu$D_large |> 
  filter(assoc.prelim %in% c("Vu/Enh-Aj")) |> 
  group_by(campagne, parcelle) |> 
  summarise() |> 
  tt()
```
...
## Attribuer le code du groupe
Commu$D_large$assoc.prelim[groupes == 5] <- "Sh"
```
...
```{r}
groupes <- cutree(Beta_flex, h = 1.6)

plot(as.hclust(Beta_flex), labels = groupes)
rect.hclust(as.hclust(Beta_flex), h = 1.6,
            border = c(rep("black", 3), 
                       "red", 
                       rep("black", length(unique(groupes)) - 4))
            )
## Attribuer le code du groupe
Commu$D_large$assoc.prelim[groupes == 16] <- 5
```
...
Table_sp |> 
  filter(assoc.prelim == "5", occ.class != "I") |> 
  arrange(sous.regne, desc(occ.pourc)) |> 
  select(sp.nom, occ.class, couvert) |> 
  tt() |> 
  setNames(c("Espèce", "Occurence", "Couvert")) |> 
...
-   Rhododendron lapponicum (arctique-alpin) : Calciphile et saxicole, le Rhododendron lapponicum occupe sporadiquement les ankers et les rendzines des affleurements granitiques et calcaires... La plante pousse en toundra sèche, exposée et peu enneigée...

## Photos

```{r}
Commu$D_large |> 
  filter(assoc.prelim == 5) |> 
  group_by(campagne, parcelle) |> 
  summarise() |> 
  tt()
```
...
```{r}
groupes <- cutree(Beta_flex, h = 1.7)

plot(as.hclust(Beta_flex), labels = groupes)
rect.hclust(as.hclust(Beta_flex), h = 1.7,
            border = c(rep("black", 4), 
                       "red", 
                       rep("black", length(unique(groupes)) - 5))
            )
## Attribuer le code du groupe
Commu$D_large$assoc.prelim[groupes == 14] <- 6
```
...
Table_sp |> 
  filter(assoc.prelim == "6", occ.class != "I") |> 
  arrange(sous.regne, desc(occ.pourc)) |> 
  select(sp.nom, occ.class, couvert) |> 
  tt() |> 
  setNames(c("Espèce", "Occurence", "Couvert")) |> 
